...
std::string RTC::getCurrentTimeString(DateTime &time) {
  ...
  hour   = (time.hour() < 10 ? "0" : "") + std::to_string(time.hour());
  minute = (time.minute() < 10 ? "0" : "") + std::to_string(time.minute());
...
void RTC::printRTCData(DateTime &startTime) {
  ...
  // calculate a date which is 7 days, 12 hours, 30 minutes, 6 seconds into the
  // future
